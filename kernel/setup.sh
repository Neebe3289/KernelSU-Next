...
setup_kernelsu() {
    ...
    test -d "$GKI_ROOT/KernelSU-Next" || git clone https://github.com/Neebe3289/KernelSU-Next && echo "[+] Repository cloned."
    cd "$GKI_ROOT/KernelSU-Next"
    git stash && echo "[-] Stashed current changes."
    if [ "$(git status | grep -Po 'v\d+(\.\d+)*' | head -n1)" ]; then
        git checkout susfs-1.5.12-test && echo "[-] Switched to susfs-1.5.12 branch."
    fi
